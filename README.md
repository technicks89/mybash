# TechNicks's `.bashrc` Configuration (Linux/Mac)
...
```sh
git clone --depth=1 https://github.com/technicks89/ama-bash.git
cd ama-bash
./setup.sh
```
...
- Creating necessary directories (`~/.local/share/ama-bash`, `~/.config/starship`, and `~/.config/fastfetch`)
- Copying the managed repository files into `~/.local/share/ama-bash`
...
- Linking configuration files from `~/.local/share/ama-bash` to your home directory
...
Use `--keep-deps` to remove the ama-bash configuration while retaining installed software and fonts.
...
- Deleting the `~/.local/share/ama-bash` directory
